...
def main(q_values, r_0_values, name, dimensions, filename=None, sphere=False, randomsphere=False, eqsphere=False,
    ...
    for r_0 in r_0_values:
        ...
        for q in q_values:
            ...
            #lam, lam2 = worker(q, L_0 = z.L_0, k = z.k, N_tot = z.N_tot, directed = directed, normalized=True)
    ...
    if rank==0:
        ...
        for r_0 in r_0_values:
            ...
            for q in q_values:
                ...
                shortdict[str(r_0)]['qs'][str(q)]['second_largest_norm'] = [np.mean(np.array(lams_norm)), np.std(np.array(lams_norm))]
                shortdict[str(r_0)]['qs'][str(q)]['smallest_norm'] = [np.mean(np.array(lams2_norm)), np.std(np.array(lams2_norm))]
