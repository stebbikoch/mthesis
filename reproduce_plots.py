...
f2 = open('./reproduce/reproduce_1000_undirected_with_averaging.txt', )
...
for k in k_values:
    ...
    line2, =plt.plot(q_values_1, [sum(data2[str(k)][str(q)])/10 for q in q_values_1], 'x', color=line1.get_color(), markerfacecolor='none')#, color=c[index])
...
legend1 = plt.legend([line1, line2], [r"Analytical prediction", r"Numerical results"])
