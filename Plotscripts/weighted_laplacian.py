...
f = open('../results/weighted_ring_undirected.jsonshort.json')
...
for r_0 in r_0_values:
    line1, =ax.plot(q_values1, [instance.exact_eigens_weighted(r_0, q) for q in q_values1], label=r'k='+str(2*r_0),linewidth=1)#, color=c[index])
    ...
    line4, = ax1.plot(q_values1, [instance.exact_eigens_weighted(r_0, q, smallest=True) for q in q_values1], label=r'k=' + str(2 * r_0), linewidth=1)
...
plt.savefig('../figures/weighted_laplacian.svg', format='svg', dpi=1000, bbox_inches='tight')
plt.show()