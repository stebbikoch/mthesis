...
f2 = open('results/test_sphere5000.jsonlong.json')
...
for k in r_values:
    print(data2[str(k)]['k'])
    line1, =plt.plot(q_values, [instance.second_lam_sphere(q=q, r_0=k) for q in q_values], label=r'k='+str(k))#, color=c[index])
    line2 =plt.scatter(q_values_1, [data2[str(k)]['qs'][str(q)] for q in q_values_1],color=line1.get_color())#, color=c[index])
